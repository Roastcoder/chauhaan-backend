...
// Helper to get Razorpay config dynamically
const getRazorpayConfig = async () => {
  try {
    const stored = await db.get("SELECT value FROM settings WHERE key = ?", ['razorpay_config']);
    if (stored) {
    ...
    console.error("Error fetching stored razorpay config:", e);
  }
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    const expectedSign = crypto
      .createHmac("sha256", config.key_secret || process.env.RAZORPAY_KEY_SECRET)
